...
final class PersistentBankAccount: Sendable {
    ...
    convenience init(from domain: BankAccount) {
        self.init(id: domain.id, userId: domain.userId, name: domain.name, balance: domain.balance, currency: domain.currency, createdAt: domain.createdAt, updatedAt: domain.updatedAt)
    }
